def input_students
  ...
  while !name.empty? do
    ...
    age = gets.chomp
    puts "Please enter student's height"
    height = gets.chomp
...
def print(students)
  ...
  while acc < students.count
    puts "#{acc + 1}: #{students[acc][:name]} 
    (#{students[acc][:cohort]} cohort, 
    Age: #{students[acc][:age]}, 
    Height: #{students[acc][:height]}, 
    Hobby: #{students[acc][:hobby]})"
    acc += 1
  end  
end
